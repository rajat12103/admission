...
<div class="card">
                            ...
                            <div class="card-body">
                             ...
                             <!-- <th class="sorting" tabindex="0" aria-controls="bootstrap-data-table-export" rowspan="1" colspan="1" aria-label="Salary: activate to sort column ascending" style="width: 108px;">Attendance</th> -->
                                    ...
                                    <tbody>   
                                    <form class="form-stacked" action="{{url('/adddocumentverification103')}}" method="post" enctype="multipart/form-data">
                                        {{csrf_field()}}
                             ...
                             <tr>
                                ...
                                @if(!empty($row->tenth_doc_present))
                                <td></td>
                                <td></td>
                                <td></td>
                                <td></td>
                                @else
                                <td>
                                    <input type="radio" name="tenth_doc_present[{{$row->rollno}}]" value="Yes" required="">Y
        <input type="radio" name="tenth_doc_present[{{$row->rollno}}]" value="No" required="">N
                                   
                                </td>
                                <td><input type="radio" name="twe_doc_present[{{$row->rollno}}]" value="Yes" required="">Y
        <input type="radio" name="twe_doc_present[{{$row->rollno}}]" value="No" required="">N</td>
        <td><input type="radio" name="migration_present[{{$row->rollno}}]" value="Yes" required="">Y
        <input type="radio" name="migration_present[{{$row->rollno}}]" value="No" required="">N</td>
        <td><input type="radio" name="service_present[{{$row->rollno}}]" value="Yes" required="">Y
        <input type="radio" name="service_present[{{$row->rollno}}]" value="No" required="">N</td>
@endif
                    ...
                    @if(!empty($row->tenth_doc_present))
                        <h4 style="text-align: center;">Document Info already submitted!</h4>
                        <br>
                        <h4 style="text-align: center;"> Click here to <a href="{{url('/editdocument102')}}">Edit Document</a></h4>
